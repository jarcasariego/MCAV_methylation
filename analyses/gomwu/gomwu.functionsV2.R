...
gomwuStats=function(input,goDatabase,goAnnotations, goDivision,Module=FALSE,
    Alternative="t", adjust.multcomp="BH", clusterCutHeight=0.25,largest=0.1,smallest=5,
    perlPath="perl",scriptPath,outputPath,
     shuffle.reps=10){
	...
	system(paste(perlPath,paste0(".",scriptPath,"gomwu_a.pl"),goDatabase,goAnnotations,input,goDivision,extraOptions))
	clusteringGOs(goAnnotations,goDivision,clusterCutHeight)
	system(paste(perlPath,paste0(".",scriptPath,"gomwu_a.pl"),goAnnotations,input,goDivision))
	...
	rr$p.adj=fdr
	fname=paste(outputPath,"MWU_",inname,sep="")
