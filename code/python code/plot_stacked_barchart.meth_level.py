...
"""
> plot_stacked_barchart.meth_level.py <

Plots stacked barchart to illustrate the changes in methylation level of repeat
elements across treatments.
"""
import csv
...
tsv_files = Path.cwd().glob('???????.meth_level.tsv')
for t in tsv_files:
    temp = pd.read_table(t, usecols=[0, 8, 15, 22])
    temp['treatment'] = t.name[:2]
    temp['replicate'] = t.name[3:7]
    df.append(temp)
...
excluded_repeats = ['ARTEFACT','Unknown','scRNA','RC','Satellite', 'Unspecified', 'all_repeats','Retroposon','srpRNA']
...
for n, r in enumerate(repeat_types):
    ...
    if n == 0:
        barh_plots[r] = plt.barh(yloc, df_means.loc['DNA'].unstack(), 0.3,
                                 color=barh_col, ecolor=barh_col,
                                 xerr=df_sems.loc['DNA'].unstack())
...
output_filename = 'stacked_barchart.meth_level.pdf'
